...
const PlacesScreens = () => {
  ...
  return (
    <View>
      ...
      <Pressable onPress={()=>setModalVisible(!modalVisible)} style={styles.Press}>
      <Octicons name="arrow-switch" size={24} color="gray" />
      <Text style={styles.PressText}>Sort</Text>
      </Pressable>

      <Pressable style={styles.Press}>
<Ionicons name="filter-outline" size={24} color="gray" />   
   <Text style={styles.PressText}>Filter</Text>
      </Pressable>

      <Pressable style={styles.Press} onPress={()=>navigation.navigate('Map',{
        SearchResults:searchPlaces
      })}>
      <FontAwesome5 name="map-marker-alt" size={24} color="gray" />      
      <Text style={styles.PressText}>Map</Text>
      </Pressable>
      </Pressable>
      <ScrollView style={styles.styleScroll}>
        {sortedData?.filter((item)=>item.place ===route.params.places)
                  ...
                  <View style={{flexDirection:'row'}}>
                    ...
                    <View style={{flex:3}}> 
                        {filters.map((item,index)=>(
                          <Pressable onPress={()=>setSelectedFilter(item.filter)} key={index} style={styles.sortFilterWrapper}>
